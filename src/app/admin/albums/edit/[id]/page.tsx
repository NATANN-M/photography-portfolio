...
import { Trash2, CheckCircle2, ChevronLeft, Upload, Grid, Settings as SettingsIcon, Image as ImageIcon, Loader2, Plus, Star } from "lucide-react";
...
export default function EditAlbumPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-24 px-4">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div className="space-y-2">
            ...
            <h1 className="text-3xl md:text-4xl font-black tracking-tighter uppercase italic">Curate Collection</h1>
        ...
        <div className="flex flex-wrap items-center gap-4">
            {selectedPhotos.length > 0 && (
                <button 
                    ...
                    className="bg-red-500 hover:bg-red-600 text-white px-6 py-4 rounded-2xl font-black uppercase tracking-widest transition-all flex items-center gap-3 shadow-xl animate-in zoom-in-95 duration-300 text-xs"
                >
            ...
            <button
                onClick={handleSave}
                disabled={saving}
                className="flex-1 lg:flex-none bg-white text-black px-10 py-4 rounded-2xl font-black uppercase tracking-widest hover:scale-105 active:scale-95 disabled:opacity-50 transition-all shadow-2xl flex items-center justify-center gap-3 text-xs"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-1 space-y-8 order-2 lg:order-1">
            <div className="bg-[#0a0a0a] p-6 md:p-10 rounded-[2.5rem] border border-white/5 space-y-8 shadow-2xl lg:sticky lg:top-10 overflow-hidden group">
                ...
                <div className="space-y-6 relative z-10">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="aspect-[4/5] rounded-3xl overflow-hidden border-2 border-white/5 relative group bg-white/5 shadow-inner">
                            {coverImage ? (
                                <img src={coverImage} alt="preview" className="w-full h-full object-cover" />
                            ) : (
                                <div className="w-full h-full flex flex-col items-center justify-center text-white/10 p-6 text-center">
                                    <ImageIcon size={32} />
                                    <span className="text-[8px] mt-4 font-black uppercase tracking-widest">No Cover Set</span>
                                </div>
                            )}
                            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center backdrop-blur-sm p-6 gap-3">
                                ...
                                <p className="text-[8px] font-black uppercase tracking-widest text-white/40">Select from grid below to use existing</p>
                            </div>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 md:gap-6">
                {photos.map((photo) => (
                    <div 
                        ...
                        className={`group relative aspect-[3/4] rounded-[1.5rem] md:rounded-[2rem] overflow-hidden border-2 transition-all duration-500 cursor-pointer shadow-xl ${
                    ...
                    >
                        ...
                        <div className="absolute inset-0 flex flex-col justify-between p-4 md:p-6">
                            <div className="flex justify-between items-start">
                                <button 
                                    onClick={(e) => { e.stopPropagation(); setAsCover(photo.imageUrl); }}
                                ...
                                >
                                    <Star size={14} fill={coverImage === photo.imageUrl ? "currentColor" : "none"} />
                                </button>
                            ...
                            <div className={`transition-all duration-500 ${selectedPhotos.includes(photo.id) ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
                                <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest truncate mb-2">{photo.title}</p>
                                <button 
                ...
                <Link 
                    ...
                    className="aspect-[3/4] rounded-[1.5rem] md:rounded-[2rem] border-2 border-dashed border-white/5 hover:border-white/20 hover:bg-white/[0.02] transition-all flex flex-col items-center justify-center gap-4 group"
                >
